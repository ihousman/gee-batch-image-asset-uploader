...
gs_bucket = 'gs://test-bucket-housman2/'
...
band_names =['code']#['blue','green','red','nir','swir1','swir2','temp']

#Directory where images are to be uploaded are located
image_dir ='Z:/TimeSync/AncillaryData/LANDFIRE/tifs/'
...
years = [int(base(i).split('_')[1][-4:]) for i in tifs]

dates = [year_month_day_to_seconds([year,6,1])*1000 for year in years]

##dates = [year_month_day_to_seconds([2000,6,1])*1000 for i in tifs]
###Get the id names
system_index_list = [base(i)[:-2] for i in tifs]
####print(system_index_list)
